...
struct BottomSheetModal: View {
        ...
        allBooths.filter { booth in
            ...
            let matchesBrand: Bool = {
                ...
                if brandOptions.contains("Flash Sale") {
                    return !(booth.flashSaleSchedule?.isEmpty ?? true)
                }
                if brandOptions.contains("Freebies") {
                    return booth.freebies
                }
                if brandOptions.contains("Hands On Tester") {
                    return booth.tester
                }
                return true
            }()
    ...
    var body: some View {
        ...
        .sheet(item: $selectedBooth) { booth in
            BrandDetailCard(booth: booth, isExpanded: $isExpanded, selectedDest: $selectedDest, landmarks: landmarks)
